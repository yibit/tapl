...
and process_command (ctx, nextuvar, constr) cmd =
  match cmd with
  | Import f -> process_file f (ctx, nextuvar, constr)
  | Eval (fi, t) ->
    let tyT, nextuvar', constr_t = recon ctx nextuvar t in
    let t' = eval ctx t in
    let constr' = combineconstr constr constr_t in
    let constr'' = unify fi ctx "Could not simplify constraints" constr' in
