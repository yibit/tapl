...
let rec eval1 ctx store t =
  ...
  | TmTApp (fi, TmTAbs (_, x, _, t11), tyT2) -> tytermSubstTop tyT2 t11, store
  | TmTApp (fi, t1, tyT2) ->
    let t1', store' = eval1 ctx store t1 in
  ...
  | TmRecord (fi, fields) ->
    let rec evalafield l =
      ...
      | (l, ti) :: rest ->
        ...
        (l, ti') :: rest, store'
    in
    let fields', store' = evalafield fields in
    TmRecord (fi, fields'), store'
  | TmProj (fi, TmRecord (_, fields), l) ->
    (try List.assoc l fields, store with
  ...
  | TmTimesfloat (fi, TmFloat (_, f1), TmFloat (_, f2)) -> TmFloat (fi, f1 *. f2), store
  ...
  | TmIsZero (_, TmSucc (_, nv1)) when isnumericval ctx nv1 -> TmFalse dummyinfo, store
  | TmIsZero (fi, t1) ->
    let t1', store' = eval1 ctx store t1 in
...
let rec kindof ctx tyT =
  ...
  | TyRecord fldtys ->
    List.iter
      (fun (l, tyS) -> if kindof ctx tyS <> KnStar then error dummyinfo "Kind * expected")
      fldtys;
    KnStar
  | TySome (tyX, knK, tyT2) ->
    let ctx' = addbinding ctx tyX (TyVarBind knK) in
    if kindof ctx' tyT2 <> KnStar then error dummyinfo "Kind * expected";
    KnStar
  | _ -> KnStar
;;
...
let rec typeof ctx t =
  ...
  | TmApp (fi, t1, t2) ->
    ...
    (match simplifyty ctx tyT1 with
     | TyArr (tyT11, tyT12) ->
       if tyeqv ctx tyT2 tyT11 then tyT12 else error fi "parameter type mismatch"
  ...
  | TmTApp (fi, t1, tyT2) ->
    ...
    (match simplifyty ctx tyT1 with
     | TyAll (_, knK11, tyT12) ->
       if knK11 <> knKT2 then error fi "Type argument has wrong kind";
  ...
  | TmProj (fi, t1, l) ->
    (match simplifyty ctx (typeof ctx t1) with
     | TyRecord fieldtys ->
       (try List.assoc l fieldtys with
  ...
  | TmPack (fi, tyT1, t2, tyT) ->
    ...
    (match simplifyty ctx tyT with
     | TySome (tyY, k, tyT2) ->
       if kindof ctx tyT1 <> k then error fi "type component does not have expected kind";
       let tyU = typeof ctx t2 in
       let tyU' = typeSubstTop tyT1 tyT2 in
       if tyeqv ctx tyU tyU' then tyT else error fi "doesn't match declared type"
