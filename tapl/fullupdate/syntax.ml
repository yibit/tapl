...
let tymap onvar c tyT =
  let rec walk c tyT =
    ...
    | TyRecord fieldtys ->
      TyRecord (List.map (fun (li, (varTi, tyTi)) -> li, (varTi, walk c tyTi)) fieldtys)
    | TyBool -> TyBool
    | TyNat -> TyNat
    | TyTop -> TyTop
    | TyId b as tyT -> tyT
...
let tmmap onvar ontype c t =
  let rec walk c t =
    ...
    | TmRecord (fi, fields) ->
      TmRecord (fi, List.map (fun (li, (vari, ti)) -> li, (vari, walk c ti)) fields)
    | TmTrue fi as t -> t
    | TmFalse fi as t -> t
