...
let rec typeof ctx t =
  ...
  | TmZero fi -> TyNat
  | TmSucc (fi, t1) ->
    if typeof ctx t1 = TyNat then TyNat else error fi "argument of succ is not a number"
  | TmPred (fi, t1) ->
    if typeof ctx t1 = TyNat then TyNat else error fi "argument of pred is not a number"
  | TmIsZero (fi, t1) ->
