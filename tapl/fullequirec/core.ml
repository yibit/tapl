...
let rec tyeqv seen ctx tyS tyT =
  ...
  ||
  match tyS, tyT with
  | TyRec (x, tyS1), _ -> tyeqv ((tyS, tyT) :: seen) ctx (typeSubstTop tyS tyS1) tyT
  | _, TyRec (x, tyT1) -> tyeqv ((tyS, tyT) :: seen) ctx tyS (typeSubstTop tyT tyT1)
  ...
  | TyVariant fields1, TyVariant fields2 ->
    List.length fields1 = List.length fields2
    && List.for_all2
         (fun (li1, tyTi1) (li2, tyTi2) -> li1 = li2 && tyeqv seen ctx tyTi1 tyTi2)
         fields1
         fields2
  | TyUnit, TyUnit -> true
  | _ -> false
     ...
     | TyVariant fieldtys ->
       ...
       let casetypes =
         List.map
           (fun (li, (xi, ti)) ->
             let tyTi =
               try List.assoc li fieldtys with
               | Not_found -> error fi ("label " ^ li ^ " not found")
             in
       ...
       List.iter
         (fun tyTi ->
           if not (tyeqv ctx tyTi tyT1) then error fi "fields do not have the same type")
         restTy;
       tyT1
