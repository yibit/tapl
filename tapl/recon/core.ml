...
let prconstr constr =
  let pc (tyS, tyT) =
    printty_Type false tyS;
    pr "=";
    printty_Type false tyT
  in
  let rec f l =
    match l with
    | [] -> ()
...
let uvargen =
  let rec f n () = NextUVar ("?X" ^ string_of_int n, f (n + 1)) in
  f 0
;;
...
let substinconstr tyX tyT constr =
  List.map (fun (tyS1, tyS2) -> substinty tyX tyT tyS1, substinty tyX tyT tyS2) constr
;;
...
let unify fi ctx msg constr =
  let rec u constr =
    ...
    | (tyS, TyId tyX) :: rest ->
      ...
      else List.append (u (substinconstr tyX tyS rest)) [ TyId tyX, tyS ]
    | (TyId tyX, tyT) :: rest ->
      if tyT = TyId tyX
      then u rest
      else if occursin tyX tyT
      then error fi (msg ^ ": circular constraints")
      else List.append (u (substinconstr tyX tyT rest)) [ TyId tyX, tyT ]
